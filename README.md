...
- 프로젝트 디자인 / 커뮤니티 & 리뷰 기능 개발
- 전체 서비스 UI 설계 및 반응형 스타일 구현
- 커뮤니티 게시판 구현 (작성, 수정, 삭제, 댓글, 카테고리 필터 포함)
- 리뷰 작성 기능 구현 (별점, 청결도 선택, 한 줄 리뷰 입력)
- Swiper 기반 이미지 업로드 및 수정 기능 구현
- 마이페이지 연동 리뷰 목록 출력 및 상세 이동 처리
- 다크모드 대응 및 카드형 리스트 UI 구성
- emit 기반 이미지 상태 동기화 및 트러블슈팅 처리
- 디자인 QA
