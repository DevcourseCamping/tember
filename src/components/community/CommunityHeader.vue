...
<template>
  ...
  >
    ...
    <div
      class="w-full relative h-[45px] flex items-center justify-between bg-[var(--white)] rounded-[5px] px-5 py-3"
    >
      <div class="flex items-center w-full">
        ...
        <input
          ...
          type="text"
          placeholder="검색하기"
          class="flex-grow h-full focus:outline-none placeholder:text-[var(--grey)] text-[15px]"
          @input="handleInput"
        />
        <!-- 20250613 검색 페이지 사용을 위한 로직 추가 -->
        <button @click="emit('iconClick')" v-if="props.iconType === 'arrow'">
          <img :src="arrow" class="w-5 h-5 ml-5" />
        </button>
        <button @click="emit('handleFilterClick')" v-else>
          <img :src="filter" class="w-5 h-5 ml-5" />
        </button>
        <!-- end -->
      </div>
    </div>
